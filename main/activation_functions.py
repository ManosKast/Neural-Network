...
def linear_activation_gradient(x):
    return 1

def binary_step_activation_gradient(x):
    return 0

def ReLU_activation_gradient(x):
    return 1 if x >= 0 else 0

def PReLU_activation_gradient(x, alpha=0.01):
    return 1 if x >= 0 else alpha

def ELU_activation_gradient(x, alpha=1.0):
    return 1 if x >= 0 else alpha * np.exp(x)

def SLU_activation_gradient(x, c=1.0):
    return np.exp(c*x) * (np.exp(c*x) + c*x + 1) / (np.exp(c*x) + 1)**2

def logistic_activation_gradient(x, c=1.0):
    return c * np.exp(-c*x) / (1 + np.exp(-c*x))**2

def softplus_activation_gradient(x):
    return 1 / (1 + np.exp(-x))

def tanh_activation_gradient(x):
    return 1 - np.tanh(x)**2

def arctan_activation_gradient(x):
    return 1 / (x**2 + 1)
